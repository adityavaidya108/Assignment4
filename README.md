...
We also built a Streamlit UI because sometimes clicking is easier than typing:
...
We added caching because OpenAI API calls aren't free. Each code snippet gets hashed, and if we've seen it before, we skip the API call and use the cached result. Saves a ton of money if you're scanning the same codebase multiple times (like during development).
